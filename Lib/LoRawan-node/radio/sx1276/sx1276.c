                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    if( ( paConfig & RF_PACONFIG_PASELECT_PABOOST ) == RF_PACONFIG_PASELECT_PABOOST )
    {
        if( power > 17 )
        {
            ...
            DEBUG(3,"paDac--1 = 0x%02x\r\n",paDac);
        }
        else
        {
            paDac = ( paDac & RF_PADAC_20DBM_MASK ) | RF_PADAC_20DBM_OFF;
            DEBUG(3,"paDac--2 = 0x%02x\r\n",paDac);
        }
	...
	DEBUG(3,"paDac = 0x%02x\r\n",paDac);
   
    SX1276Write( REG_PADAC, paDac ); ///当PA = 20时 paDac = 0x87，符合开启高PA要求
    ...
    if( SX1276.Settings.Modem == MODEM_FSK )
    {
        ...
        if( rxContinuous == false )
        {
            TimerSetValue( &RxTimeoutSyncWord, ( 8.0 * ( SX1276.Settings.Fsk.PreambleLen +
                                                         ...
                                                         1.0 ) + 10.0 ) /
                                                        ( double )SX1276.Settings.Fsk.Datarate ) * 1e3 );
...
{
    // Set RESET pin to 0
    GpioWrite( GPIOB,LORA_REST_PIN,GPIO_PIN_RESET );
	
    // Wait 1 ms
    DelayMs( 1 );

    // Set SET pin to 1
    GpioWrite( GPIOB,LORA_REST_PIN,GPIO_PIN_SET );
...
void SX1276OnTimeoutIrq( void )
{
    ...
    case RF_TX_RUNNING:
        ...
        if( ( RadioEvents != NULL ) && ( RadioEvents->TxTimeout != NULL ) )
        {
            ...
            DEBUG_ERROR(2,"Txtimeout LoRa Hardware abnormal\r\n");
        }
        break;
    default:
        break;
...
extern void OnRadioCadDone( bool channelActivityDetected );
...
void SX1276OnDio1Irq( void )
{
	DEBUG(2,"%s %d\r\n",__func__, SX1276.Settings.State);
    switch( SX1276.Settings.State )
    {                
        case RF_RX_RUNNING:
            switch( SX1276.Settings.Modem )
