    ...
    switch( macHdr.Bits.MType )
    {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            break;
				...
				case FRAME_TYPE_DATA_CONFIRMED_UP:  ///增加节点间通信数据接收处理		
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {					
                ...
                address |= ( (uint32_t)payload[pktHeaderLen++] << 24 );
							  ...
							  /**************************** add buy Jason Start **********************************/
							  ///分为两种情况处理：
								//1：接收到Mac地址是广播地址：FF-FF-FF-FF，此时设备唤醒接收数据，正常处理广播数据；
							  //2：侦听模式区分为两种：数据正常接收；接收数据CRC_ERROR
  							//： 侦听模式下接收到Mac地址，所以此时存在干扰，需要进行规避，两次规避后强制开启发送数据，降低发送数据时延					
							
                if( address != LoRaMacDevAddr ) ///直接处理广播ID，非广播则过滤，不做处理，广播ID： FF-FF-FF-FF，只用于C类
                {
									
									///优化框架：可扩展性考虑
										if( LoRaMacDevAddr != 0xFFFFFFFF ) ///干扰信号：随机发送期间侦听到信号，则规避，下个随机时间再发送数据
										{               
												LoRaMacCsma.ListenAagain(  );
										}
										///  else if() send Done 则不是侦听模式，处于接收模式接收到非网关应答数据，需要开启重发保护机制
										///  不才用该模式进行直接重发，结构完整性考虑，调用CLASS A confired mode处理
										else if( LoRaMacDevAddr != 0xFFFFFFFF && !Csma.Listen )
										{
												...
												return; ///接收到非广播数据，直接退出
										}
										
										else if( LoRaMacDevAddr == 0xFFFFFFFF ) ///广播数据处理
									{

										 DEBUG(3,"22--Address : %08x address : %08x\r\n",MulticastChannels->Address,address);
										 MulticastChannels->Address = address;
										 curMulticastParams = MulticastChannels;

										 while( curMulticastParams != NULL )
										{
												 DEBUG(3,"33--Address : %08x address : %08x\r\n",curMulticastParams->Address,address);
									...
									if( multicast == 0 )
									{
											...
											DEBUG_WARNING(2,"LoRaCsma.Listen RxDoneAbort return\r\n");
												return;
									}
								}
								else
								{
									...
									nwkSKey = LoRaMacNwkSKey;
									appSKey = LoRaMacAppSKey;
									downLinkCounter = DownLinkCounter;  ///默认都是0
								}
						...
						fCtrl.Value = payload[pktHeaderLen++]; ///FCtrl bits: 0x20表示为ACK

						DEBUG(2,"pktHeaderLen = %d %02x\r\n",pktHeaderLen,fCtrl.Value);

										sequenceCounter = ( uint16_t )payload[pktHeaderLen++];     ///接收到数据帧号
						...
						sequenceCounterPrev = downLinkCounter; ///保留上次接收到的帧号：只接收一个终端设备是正常的

						DEBUG(3,"sequenceCounterPrev111 = %d\r\n",sequenceCounterPrev);
						
						if(sequenceCounter < sequenceCounterPrev) ///当接收到多个终端发送到的数据时必须重置上次接收帧号，否则会出现异常MIC解密失败
						{      	
											sequenceCounterPrev = downLinkCounter = 0;  
							DEBUG(3,"sequenceCounterPrev = %d\r\n",sequenceCounterPrev);
						}
						
						sequenceCounterDiff = ( sequenceCounter - sequenceCounterPrev );  ///丢包数
            ...
            if( sequenceCounterDiff < ( 1 << 15 ) ) ///:16位
					{
						downLinkCounter += sequenceCounterDiff; ///++1
						DEBUG(3,"DOWN_LINK = %d downLinkCounter = %d \r\n",DOWN_LINK,downLinkCounter);
						LoRaMacComputeMic( payload, size - LORAMAC_MFR_LEN, nwkSKey, address, DOWN_LINK, downLinkCounter, &mic );///MIC校验: 校验mac+paload----MIC之前数据
						DEBUG(3,"mic --- %02x\r\n",mic);
						...
						if( micRx == mic )
           {
				...
				//	McpsIndication.DownLinkCounter = downLinkCounter;
							...
							//	AddMacCommand( MOTE_MAC_LINK_ADR_ANS, 0x07, snr ); ///做假设如果网关发送ADR_req,节点没上发ADR_ANS则会导致网关不断下发数据，假设成立
							//当MIC正确后，服务器则不再显示终端ANS错误
							PrepareRxDoneAbort( );
              return;
            }
         }
...
static void OnRadioRxError( void ) ///通讯竞争碰撞
{
	///无论A/C类型，都可能出现rxerror：
	///A: 接收数据发生碰撞: 开启重发机制
	///C: 侦听/接收数发生碰撞: 再次侦听/开启重发机制
	if( LoRaMacDeviceClass != CLASS_C )
    {
    	DEBUG_ERROR(2,"OnRadioRxError\r\n");
      Radio.Sleep(  );
    }
    else
    {
    /**********************add buy Jason Start************************/
		...
		{
			///侦听阶段数据:
			DEBUG_ERROR(2,"CsmaTimerEvent OnRadioRxError\r\n");	 
			LoRaMacCsma.ListenAagain(  );					
		}
...
static void OnMacStateCheckTimerEvent( void ) 
{
    ...
    if( LoRaMacFlags.Bits.MacDone == 1 )
    {
		...
		DEBUG(2, "LoRaMacFlags.Bits.McpsInd = %d\r\n",LoRaMacFlags.Bits.McpsInd);
		DEBUG(2, "AckReceived = %d,AckTimeoutRetriesCounter = %d,AckTimeoutRetries = %d\r\n",McpsConfirm.AckReceived,AckTimeoutRetriesCounter,AckTimeoutRetries);
        ...
        if( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & MAC_TX_DELAYED ) == 0 ) )
        {
            ...
            if( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) )
            {
                ...
                LoRaMacFlags.Bits.MacDone = 0;
							 ...
							 TimerStop( &CsmaTimer );
							 TimerSetValue( &CsmaTimer, 500 + randr(-5, 5)*100); ///ALOHA
							 TimerStart( &CsmaTimer );

							 while(!Csma.Listen);
								
							 DEBUG_WARNING(2,"---ScheduleTx---\r\n");       
                ...
                /************************* add buy Jason Start************************/                              
                if(LoRapp_Handle.Send_again) ///接收数据失败，重发机制
                {
                    // Sends the same frame again
                    DEBUG(2,"ScheduleTx Start\r\n");
                    TimerStop( &CsmaTimer );
                    TimerSetValue( &CsmaTimer, 500 + randr(-5, 5)*100); ///ALOHA
                    TimerStart( &CsmaTimer );
...
//                    BoardSleep(  ); ///不需要重发
                    ...
                    ScheduleTx( );
                    SendDoneLed( ); ///发送完成闪烁 
                    LoRapp_Handle.Send_again = false;
                }  
                else ///接收到数据
                {
                    LoRaMacState &= ~MAC_TX_RUNNING;
                    ...
                    DEBUG(2,"LoRaMacState &= ~MAC_TX_RUNNING--7 %d\r\n",LoRaMacState);
                }
                /************************* add buy Jason End************************/  
#endif
            }
...
void OnRxWindow1TimerEvent( void )
{
	...
	Freq += 3e7;
    RxWindowSetup( Freq, datarate, bandwidth, symbTimeout, true );//false---true连续接收
...
///add by Ysheng End
    ...
    else
    {
				if( LoRapp_Handle.Work_Mode == CSMA ) ///侦听模式：同频
				{
					...
					DEBUG(2,"RX2Frequency CSMA = %d, Datarate = %d\r\n",Channels[Channel].Frequency,datarate);
				}
				else ///网关通讯
				{
					 RxWindowSetup( Freq, datarate, bandwidth, symbTimeout, true );
    ...
    if( Radio.GetStatus( ) == RF_IDLE )
    {
        ...
        else
        {
            modem = MODEM_LORA;
						...
						{
							 Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 202, timeout, false, 0, false, 0, 0, false, rxContinuous ); //false -- P2P
...
static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen )
{
    ...
    else
    {
        maxN = MaxPayloadOfDatarate[datarate];
				LoRapp_Handle.ADR_Datarate = datarate; ///datarate回调使用
    }
